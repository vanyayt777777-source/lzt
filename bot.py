...
user_mailing = {}
user_temp = {}
user_chats_pagination = {}  # Для пагинации чатов
...
# Функция отображения чатов с пагинацией
def get_chats_page(user_id, page=0):
    conn = sqlite3.connect('vest_soft.db')
    c = conn.cursor()
    ...
    # Пагинация по 10 чатов
    per_page = 10
    total_pages = (len(all_chats) + per_page - 1) // per_page
    page = max(0, min(page, total_pages - 1))
    
    start = page * per_page
    end = start + per_page
    chats = all_chats[start:end]
    
    return chats, page, total_pages
...
async def create_mailing(event):
    ...
    await show_chats_page(event, user_id, 0)

async def show_chats_page(event, user_id, page):
    """Показывает страницу с чатами"""
    chats, current_page, total_pages = get_chats_page(user_id, page)
    ...
    if user_id in user_mailing:
        user_mailing[user_id]['page'] = current_page
        if 'chats_data' not in user_mailing[user_id]:
            user_mailing[user_id]['chats_data'] = {}
        
        # Сохраняем чаты текущей страницы
        for chat_id, title, chat_type in chats:
            user_mailing[user_id]['chats_data'][chat_id] = (title, chat_type)
    ...
    selected = user_mailing[user_id]['selected_chats'] if user_id in user_mailing else []
    
    buttons = []
    for chat_id, title, chat_type in chats:
        emoji = "👤" if chat_type == 'user' else "👥" if chat_type == 'group' else "📢"
        short_title = title[:30] + "..." if len(title) > 30 else title
        status = "✅ " if chat_id in selected else ""
        buttons.append([Button.text(f"{status}{emoji} {short_title}")])
    
    # Кнопки навигации
    nav_buttons = []
    if current_page > 0:
        nav_buttons.append(Button.text("⬅️ Назад"))
    if current_page < total_pages - 1:
        nav_buttons.append(Button.text("➡️ Вперед"))
    
    if nav_buttons:
        buttons.append(nav_buttons)
    
    buttons.append([Button.text(f"✅ Продолжить ({len(selected)} выбрано)")])
    buttons.append([Button.text("◀️ Отмена")])
...
async def mailing_status(event):
    ...
    if not mailings:
        await event.respond("📊 У вас нет рассылок")
        return
...
async def back_to_main(event):
    ...
    if user_id in user_chats_pagination:
        del user_chats_pagination[user_id]
...
# Обработчик выбора чатов с пагинацией
@bot.on(events.NewMessage)
async def handle_chat_selection(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'select_chats':
        ...
        if text == "⬅️ Назад":
            current_page = user_mailing[user_id].get('page', 0)
            await show_chats_page(event, user_id, current_page - 1)
            return
        ...
        if text.startswith("✅ Продолжить"):
            ...
            await event.respond(
                ...
                f"Шаг 2: Введите текст сообщения для рассылки (или отправьте /skip чтобы пропустить):"
            )
            return
        
        # Проверяем, является ли текст выбором чата
        if user_id in user_mailing and 'chats_data' in user_mailing[user_id]:
            for chat_id, (title, chat_type) in user_mailing[user_id]['chats_data'].items():
                emoji = "👤" if chat_type == 'user' else "👥" if chat_type == 'group' else "📢"
                short_title = title[:30] + "..." if len(title) > 30 else title
                
                # Проверяем с префиксом ✅ или без
                if text == f"{emoji} {short_title}" or text == f"✅ {emoji} {short_title}":
                    ...
                    current_page = user_mailing[user_id].get('page', 0)
                    await show_chats_page(event, user_id, current_page)
                    return
...
async def handle_mailing_message(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'enter_message':
        ...
        if text == "/skip":
            user_mailing[user_id]['message'] = ""  # Пустое сообщение
        else:
        ...
        await event.respond(
            ...
            f"Шаг 3: Введите задержку между сообщениями (в секундах, от 1 до 60)\n"
            f"Например: 5"
        )
...
async def handle_delay(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'enter_delay':
        ...
        try:
            ...
            if delay < 1:
                delay = 1
                await event.respond("⏱ Задержка не может быть меньше 1 секунды, установлено 1 сек")
            if delay > 60:
                delay = 60
                await event.respond("⏱ Задержка не может быть больше 60 секунд, установлено 60 сек")
        except ValueError:
...
async def start_mailing(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'confirm':
        ...
        if not accounts:
            await event.respond("❌ Нет доступных аккаунтов для рассылки")
            return
...
# Обработчик ввода номера и кодов
@bot.on(events.NewMessage)
async def handle_auth_input(event):
    ...
    if text.startswith('/') or text in ['⬅️ Назад', '➡️ Вперед', '✅ Продолжить']:
        return
    ...
    if user_id in user_temp and user_temp[user_id].get('action') == 'waiting_phone':
        ...
        if not phone.startswith('+'):
            await event.respond("❌ Номер должен начинаться с '+'")
            return
        
        # Убираем все кроме цифр и +
        cleaned = ''.join(c for c in phone if c.isdigit() or c == '+')
        if len(cleaned) < 10 or len(cleaned) > 15:
            await event.respond("❌ Неверная длина номера. Пример: +79123456789")
            return
