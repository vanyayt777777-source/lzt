...
from pyrogram.errors import FloodWait, SessionPasswordNeeded, UsernameInvalid, PeerIdInvalid
...
PHOTOS_DIR = "photos"
CHATS_FILE = "chats_data.json"  # Для сохранения чатов
...
class ChatManager:
    def __init__(self):
        ...
        self.selected_chats = {}  # {user_id: [chat_ids]}
        self.mailing_tasks = {}  # {user_id: asyncio.Task}
            ...
            for chat_id in chats:
                ...
                for i in range(count):
                    ...
                    try:
                        ...
                        # Задержка между сообщениями в один чат
                        if i < count - 1:
...
class DialogueManager:
    ...
    def load_dialogues(self) -> List[List[Dict]]:
        ...
        if os.path.exists(MESSAGES_FILE):
            ...
            # Здесь должны быть загруженные диалоги
            dialogues = [
                # Диалог 1
                [
...
async def handle_callback(client: Client, callback_query: CallbackQuery):
    ...
    if data == "main_menu":
        ...
        session = mailing_data.get(chat_id, {}).get('account', '')
        ...
        if not active_warmups and not active_mailings:
...
if __name__ == "__main__":
    ...
    print("• Загрузка чатов аккаунта")
    print("• Изменение имени и описания")
    print("• Подсчет пользователей в помощи")
    app.run()
