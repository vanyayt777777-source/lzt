...
# Функция загрузки чатов аккаунта
async def load_account_chats(user_id, account_id, account_session):
    try:
        ...
        for dialog in dialogs:
            ...
            try:
                c.execute('''INSERT OR IGNORE INTO chats 
                            ...
                            VALUES (?, ?, ?, ?, ?)''',
                         (user_id, account_id, dialog.id, dialog.name, chat_type))
                chats_count += 1
            except:
                pass
...
async def load_chats(event):
    ...
    user_mailing[user_id] = {'step': 'loading_chats'}
    
    buttons = []
    for acc_id, phone in accounts:
        buttons.append([Button.text(f"📱 Загрузить {phone}")])
    buttons.append([Button.text("◀️ Отмена")])
    
    await event.respond(
        "👥 **Загрузка чатов**\n\n"
        "Выберите аккаунт для загрузки чатов:",
        buttons=buttons,
        parse_mode='md'
    )
...
async def create_mailing(event):
    ...
    chats = c.fetchall()[:20]  # Максимум 20 чатов для выбора
    conn.close()
    ...
    for chat_id, title, chat_type in chats:
        ...
        buttons.append([Button.text(f"{emoji} {title[:30]}")])
...
async def handle_chat_loading(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'loading_chats':
        if text.startswith('📱 Загрузить '):
            phone = text.replace('📱 Загрузить ', '')
            
            # Получаем ID аккаунта
            conn = sqlite3.connect('vest_soft.db')
            c = conn.cursor()
            c.execute("SELECT id, session_string FROM accounts WHERE user_id = ? AND phone = ?", (user_id, phone))
            result = c.fetchone()
            conn.close()
            
            if result:
                acc_id, session = result
                
                await event.respond(f"⏳ Загружаю чаты для {phone}...")
                
                success, message = await load_account_chats(user_id, acc_id, session)
...
async def start_mailing(event):
    ...
    if user_id in user_mailing and user_mailing[user_id].get('step') == 'confirm':
        ...
        c.execute('''INSERT INTO mailings 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)''',
                 (user_id, f"Рассылка от {datetime.now().strftime('%d.%m.%Y %H:%M')}", 
                  data['message'], len(data['selected_chats']), data['delay'], 'active'))
...
# Обработка ввода номера и кодов (предыдущий код остается)
@bot.on(events.NewMessage)
