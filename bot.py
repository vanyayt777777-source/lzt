...
class AccountManager:
    ...
    async def add_account_from_string(self, session_string: str, session_name: str = None) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            if not session_name:
                # Генерируем имя на основе временной метки
    ...
    async def add_account_by_session_file(self, file_path: str, filename: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not me:
                await client.disconnect()
                if os.path.exists(dest_path if filename.endswith('.session') else os.path.join(SESSIONS_DIR, f"{session_name}.session")):
                    os.remove(dest_path if filename.endswith('.session') else os.path.join(SESSIONS_DIR, f"{session_name}.session"))
    ...
    async def add_account(self, session_name: str, behavior_randomizer: BehaviorRandomizer) -> bool:
        ...
        try:
            ...
            if not os.path.exists(f"{session_path}.session"):
                logger.error(f"Файл сессии {session_name}.session не найден")
                return False
...
if __name__ == "__main__":
    ...
    print("• Аккаунты правильно добавляются в список")
    print("• Поддержка .txt файлов со строкой сессии")
