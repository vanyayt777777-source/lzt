...
class AccountManager:
    ...
    async def add_account_by_session_file(self, file_path: str, filename: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return False, f"❌ Ошибка кодировки файла. Убедитесь, что файл в UTF-8 кодировке."
        except Exception as e:
...
if __name__ == "__main__":
    # Устанавливаем библиотеку chardet если её нет
    try:
        import chardet
    except ImportError:
    ...
    print("• Проверка спам-блока с подробным отчетом")
    print("• При добавлении - простое приветствие")
    print("• Аккаунты правильно сохраняются по номеру")
    print("• Поддержка .txt файлов с автоопределением кодировки")
    print("• Отдельная кнопка для проверки спам-блока")
    print("• Статус спам-блока отображается в списке аккаунтов")
    app.run()
