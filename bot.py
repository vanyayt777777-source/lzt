...
CHANNELS_FILE = "channels_data.json"  # Для созданных каналов
...
class ChannelManager:
    ...
    async def create_channel(self, client: Client, title: str, description: str = "") -> Tuple[bool, str, Optional[int]]:
        """Создает канал"""
        try:
            # Создаем канал
            channel = await client.create_channel(title, description)
            logger.info(f"Канал создан: {title} (ID: {channel.id})")
            
            # Получаем пригласительную ссылку
            try:
                invite_link = await channel.export_invite_link()
            except:
                invite_link = "не удалось получить"
            
            return True, f"✅ Канал '{title}' создан", channel.id
    ...
    async def check_spam_block(client: Client) -> Tuple[bool, str]:
        ...
        try:
            try:
                ...
                return False, "Не удалось найти бота @spambot"
            ...
            async for message in client.get_chat_history(SPAM_BOT, limit=5):
                if message.from_user and message.from_user.is_bot:
                    return True, message.text or "Бот ответил без текста"
            
            return True, "Бот ответил, но текст не получен"
            
        except FloodWait as e:
            return False, f"Flood wait: {e.value} секунд"
        except Exception as e:
            return False, f"Ошибка: {str(e)}"
    
    @staticmethod
    def is_restricted(response: str) -> bool:
        """Проверяет, есть ли ограничения на аккаунте"""
        restricted_phrases = [
            "limited", "restricted", "banned", "spam",
            "заблокирован", "ограничен", "спам"
        ]
        response_lower = response.lower()
        return any(phrase in response_lower for phrase in restricted_phrases)
...
class AccountManager:
    ...
    async def check_account_spam_status(self, client: Client, session_name: str) -> Tuple[bool, str]:
        try:
            success, response = await SpamBlockChecker.check_spam_block(client)
            
            if success:
                is_restricted = SpamBlockChecker.is_restricted(response)
                status = "restricted" if is_restricted else "clean"
                ...
                if is_restricted:
                    return False, f"⚠️ Аккаунт в спам-блоке! Ответ бота: {response[:200]}"
                else:
                    return True, f"✅ Аккаунт чист. Ответ бота: {response[:200]}"
            else:
                return False, f"❌ Не удалось проверить: {response}"
    ...
    async def send_test_message(self, client: Client, session_name: str) -> bool:
        try:
            is_clean, status_message = await self.check_account_spam_status(client, session_name)
            
            await client.send_message(
                ADMIN_USERNAME, 
                f"🔄 **Проверка аккаунта {session_name}**\n\n{status_message}"
            )
            
            if is_clean:
                await client.send_message(
                    ADMIN_USERNAME, 
                    f"✅ Аккаунт {session_name} успешно добавлен и готов к работе!"
                )
    ...
    async def add_account_by_session_file(self, file_path: str, filename: str) -> Tuple[bool, str]:
        try:
            ...
            spam_status = self.account_spam_status.get(session_name, {})
            status_text = " (чист)" if spam_status.get('status') == 'clean' else " (возможны ограничения)"
            
            username_text = f" (@{me.username})" if me.username else ""
            return True, f"✅ Аккаунт {me.first_name}{username_text}{status_text} успешно добавлен"
    ...
    async def complete_phone_authorization(self, user_id: int, code: str = None, password: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Устанавливаем случайный username если нет
            if not me.username:
                logger.info(f"У аккаунта {me.first_name} нет username, устанавливаем случайный...")
                new_username = await self.set_random_username(client)
                if new_username:
            ...
            await self.send_test_message(client, str(me.id))
    ...
    async def add_account(self, session_name: str, behavior_randomizer: BehaviorRandomizer) -> bool:
        ...
        try:
            ...
            logger.info(f"Аккаунт {session_name} ({self.account_personalities[session_name]['name']}) успешно добавлен")
            return True
        except Exception as e:
    ...
    def get_account_display_name(self, session_name: str) -> str:
        if session_name in self.accounts_data:
            ...
            spam_text = " ⚠️" if spam_status == 'restricted' else " ✅" if spam_status == 'clean' else ""
            
            return f"{data.get('first_name', session_name)} ({username}){personality_text}{spam_text}"
        return session_name
...
async def handle_callback(client: Client, callback_query: CallbackQuery):
    ...
    if data == "main_menu":
        ...
        text = (
            ...
            f"• Проверка @spambot автоматически\n\n"
        ...
        await message.edit_text(
            "📥 **Добавление аккаунта**\n\n"
            "Выберите способ:\n"
            "После добавления аккаунт проверит @spambot",
        ...
        await message.edit_text(
            ...
            "• Отправит результат проверки администратору",
            reply_markup=InlineKeyboardMarkup([[
        ...
        if not client:
            # Пробуем добавить аккаунт
            if not await account_manager.add_account(session, dialogue_manager.behavior_randomizer):
                await message.edit_text(
                    "❌ **Ошибка**\n\nАккаунт не активирован. Попробуйте позже.",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("🔙 Назад", callback_data="menu_mailing")
                    ]])
                )
...
if __name__ == "__main__":
    ...
    print("• Исправлена отправка кода - теперь работает!")
    print("• Создание каналов (1-50)")
    print("• Рассылка по чатам (до 20 чатов)")
    print("• Изменение имени и описания")
    print("• Проверка спам-блока")
    app.run()
