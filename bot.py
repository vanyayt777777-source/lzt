...
class AccountManager:
    ...
    def parse_session_string(self, session_string: str) -> str:
        """Парсит строку сессии, убирая лишние символы"""
        # Убираем пробелы и переносы строк
        session_string = session_string.strip()
        
        # Если строка начинается с "1", оставляем как есть (это pyrogram session)
        ...
        try:
            ...
            if re.match(r'^[A-Za-z0-9+/=]+$', session_string):
    ...
    async def add_account_by_session_file(self, file_path: str, filename: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Если это .txt файл со строкой сессии
                ...
                # Ищем строку сессии (обычно это длинная строка)
                lines = content.split('\n')
                session_string = None
                
                for line in lines:
                    line = line.strip()
                    if line and len(line) > 50:  # Строка сессии обычно длинная
                        session_string = line
                        break
                
                if not session_string:
                    # Если не нашли длинную строку, берем первую непустую
                    for line in lines:
                        if line.strip():
                            session_string = line.strip()
                            break
                
                if not session_string:
                    return False, "❌ Файл не содержит строки сессии"
                
                # Парсим строку сессии
                session_string = self.parse_session_string(session_string)
                logger.info(f"Строка сессии (первые 50 символов): {session_string[:50]}...")
            ...
            # Отключаем клиент (но сохраняем его в менеджере)
            await client.disconnect()
            ...
            # Активируем аккаунт в менеджере
            await self.add_account(session_name)
            
            username_text = f" (@{me.username})" if me.username else ""
            return True, f"✅ Аккаунт {me.first_name}{username_text} успешно добавлен и активирован"
    ...
    async def complete_phone_authorization(self, user_id: int, code: str = None, password: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Копируем .session файл из временной папки
            ...
            if os.path.exists(temp_session_file):
                ...
                logger.error(f"Временный файл сессии не найден: {temp_session_file}")
                # Пробуем сохранить текущую сессию
                await client.storage.save()
                # Ищем файл в текущей директории
                current_session = f"{auth_data['session_name']}.session"
                if os.path.exists(current_session):
                    shutil.copy2(current_session, dest_session_file)
                    logger.info(f"Сессия скопирована из {current_session}")
    ...
    async def add_account(self, session_name: str, behavior_randomizer: BehaviorRandomizer = None) -> bool:
        """Добавляет аккаунт в менеджер для использования"""
        if session_name in self.accounts:
            logger.info(f"Аккаунт {session_name} уже в менеджере")
            return True
        ...
        try:
            ...
            # Присваиваем характер, если передан randomizer
            if behavior_randomizer and session_name not in self.account_personalities:
            ...
            logger.info(f"Аккаунт {session_name} добавлен в менеджер")
    ...
    async def remove_account(self, session_name: str):
        ...
        if session_name in self.accounts:
            ...
            logger.info(f"Аккаунт {session_name} удален из менеджера")
        ...
        if os.path.exists(session_path):
            ...
            logger.info(f"Файл сессии {session_name}.session удален")
        
        if session_name in self.accounts_data:
            del self.accounts_data[session_name]
            self.save_accounts_data()
            logger.info(f"Аккаунт {session_name} удален из данных")
    ...
    def get_account_display_name(self, session_name: str) -> str:
        ...
        if session_name in self.accounts_data:
            ...
            # Добавляем индикатор, активен ли аккаунт в менеджере
            active_icon = " 🔵" if session_name in self.accounts else " ⚪"
            
            return f"{source_icon} {data.get('first_name', session_name)} ({username}){personality_text}{spam_icon}{active_icon}"
        return session_name
...
class WarmupManager:
    ...
    async def _warmup_process(self, chat_id: int, accounts: List[str], warmup_id: str):
        try:
            # Проверяем, что все аккаунты активны
            for account in accounts:
                if account not in self.account_manager.accounts:
                    success = await self.account_manager.add_account(account, self.dialogue_manager.behavior_randomizer)
                    if not success:
                        logger.error(f"Не удалось активировать аккаунт {account} для прогрева")
                        return
...
async def start_command(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        f"{photos_warning}\n\n"
        f"✨ **Новые функции:**\n"
        f"• Аккаунты теперь автоматически активируются\n"
        f"• Исправлена загрузка .txt файлов\n"
        f"• Аккаунты по номеру теперь работают\n"
...
async def handle_callback(client: Client, callback_query: CallbackQuery):
    ...
    if data == "main_menu":
        ...
        if session not in account_manager.accounts:
            success = await account_manager.add_account(session)
            if not success:
                await message.edit_text(
        ...
        await message.edit_text(
            ...
            "После добавления аккаунт отправит простое приветствие и автоматически активируется",
            reply_markup=InlineKeyboardMarkup(buttons)
        )
        await callback_query.answer()
        ...
        await message.edit_text(
            ...
            "• Активируется для использования",
            reply_markup=InlineKeyboardMarkup([[
        ...
        await message.edit_text(
            ...
            "Файл должен содержать строку сессии (одна строка).\n\n"
            "Поддерживаются все форматы:\n"
            "• Pyrogram строка (начинается с '1')\n"
            "• Base64 строка\n"
            "• Любая другая строка сессии\n\n"
            "Автоматическое определение кодировки.\n\n"
            "После загрузки аккаунт автоматически активируется.\n\n"
        ...
        if session not in account_manager.accounts:
            ...
            if not success:
                await message.edit_text(
                    "❌ **Ошибка**\n\nНе удалось активировать аккаунт для редактирования.",
                    reply_markup=InlineKeyboardMarkup([[
...
if __name__ == "__main__":
    ...
    print("• Аккаунты теперь автоматически активируются после добавления")
    print("• В списке аккаунтов видно, какие аккаунты активны (🔵)")
    print("• Исправлена ошибка 'не удалось активировать аккаунт'")
    print("• Аккаунты по номеру теперь работают и отображаются")
